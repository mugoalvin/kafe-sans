...
const Trigger = ({ children }: PropsWithChildren) => {
    ...
    return (
        <>
            <div onClick={toggleOpen} style={{height : '100%'}} >{children}</div>
...
const Content = ({ align = 'right', width = '48', contentClasses = 'py-1 bg-white dark:bg-gray-700', children }: PropsWithChildren<{ align?: 'left'|'right', width?: '48', contentClasses?: string }>) => {
    ...
    return (
        <>
            ...
            >
                <div className={`absolute z-50 mt-2 rounded-md shadow-lg ${alignmentClasses} ${widthClasses}`} onClick={() => setOpen(false)} id='dropDownBody'>
    ...
    return (
        <Link
            ...
            className={
                ...
                'block w-full px-4 py-2 text-start text-sm  leading-5 text-gray-600 dark:text-gray-500 hover:bg-gray-400 dark:hover:bg-gray-300 focus:outline-none focus:bg-gray-100 dark:focus:bg-gray-600 transition duration-150 ease-in-out' +
                className
            }
        >
            {children}
